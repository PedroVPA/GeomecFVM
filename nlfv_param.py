...
class set_nlfv_misc:
    ...
    def interface(self,mesh):
        ...
        self.interface_elems = interface_elem

    def normals(self,mesh):

        interface = self.interface_elems
    ...
    def heights(self,mesh):
        ...
        interface = self.interface_elems[nbe:]
        ...
        hlef = np.linalg.norm(vece,axis=1)/np.linalg.norm(IJ,axis=1)
    ...
    def edge_ordering(self,mesh):
        ...
        # Ordenar as arestas do elemento em sentido anti-horário
        ...
        if shape_array.max() == 3:
            ...
            sort_elem = np.take_along_axis(elem, np.argsort(angle), axis=1)

            auxcoord1 = (coord[sort_elem[:,0],:] + coord[sort_elem[:,1],:])/2
            auxcoord2 = (coord[sort_elem[:,1],:] + coord[sort_elem[:,2],:])/2 
            auxcoord3 = (coord[sort_elem[:,2],:] + coord[sort_elem[:,3],:])/2 
            auxcoord4 = (coord[sort_elem[:,3],:] + coord[sort_elem[:,0],:])/2 
        ...
        if shape_array.max() == 2:
            ...
            auxcoord1 = (coord[sort_elem[:,0],:] + coord[sort_elem[:,1],:])/2
            auxcoord2 = (coord[sort_elem[:,1],:] + coord[sort_elem[:,2],:])/2 
            auxcoord3 = (coord[sort_elem[:,2],:] + coord[sort_elem[:,0],:])/2 
...
class set_nlfv_flux:
    ...
    def __init__(self,mesh,tensor,misc) -> None:
        ...
        self.weights(mesh,misc)
        self.harmonic_points(mesh,misc)
        self.transmissibility(mesh,misc)
    ...
    def harmonic_points(self,mesh,misc):
        ...
        interface = misc.interface_elems[nbe:]
...
class set_nlfv_stress:
    ...
    def __init__(self,mesh,tensors,misc) -> None:
        ...
        misc.conormal(mesh, tensors.xy)
        Cxy_nij = misc.co_normal
        Cxyn = misc.normal_proj

        misc.conormal(mesh, tensors.yx)
        Cyx_nij = misc.co_normal
        Cyxn = misc.normal_proj

        misc.conormal(mesh, tensors.yy)
    ...
    def transmissibility(self,mesh,misc):
        ...
        if ksi_ph_xx.trans.max() > 1e29:

            points = mesh.edges.center[:]
            ksi_ph_xx = transmissibilities(mesh,misc,points,K_nij)
...
class transmissibilities:
    ...
    def computation(self, mesh, misc, points, K_nij, bedge, row):
        ...
        if bedge == 0:
            ...
            elemen = misc.interface_elems[:][:nbe,row]
...
# Classes down below are ideas not yet implemented


# Faster testing of the harmonic points pairs ===============================
...
class nlfv_misc:
    ...
    def conormal(self,mesh,tensor,misc):
        ...
        interface = misc.interface_elems
